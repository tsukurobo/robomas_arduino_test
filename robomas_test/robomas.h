//robomas.h
//C610_引数のidはC610に割り振られたidであることに注意。(1から始まる)
...
public:
    ...
    // 電流値の最大値を設定する。
    static void setMaxCurrent(unsigned int C610_id, uint16_t max);

    // idに対して動作電流値を設定する。
    // 電流値の範囲は-10000mA~10000mA
    // PID制御しやすいようにdouble型を引数としているが16bitの整数に変換されるため小数点以下は反映されない。
    static void put(unsigned int C610_id, double current);
    ...
    // 機械角度範囲:0~2π
    // 単位 rad
    static double getAngle(unsigned int C610_id);

    // モーター回転速度を取得
    // 単位 rpm
    static int16_t getVelocity(unsigned int C610_id);

    // 実際のトルク電流を取得
    // 単位 mA
    static int16_t getTorqueCurrent(unsigned int C610_id);
};
